...
frames the reference `capnp` CLI encoded
(`tests/fixtures/rpc-{provide,accept,introduce}.bin`, regenerated by
`scripts/gen-rpc-frames.sh`): a layout the vat shares with its own
builder but the wire format does not would pass every self-built test.
